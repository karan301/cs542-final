...
img_data_list3=[]
data_dir_list3 = os.listdir(testblur_directory)

img_list3=os.listdir(testblur_directory)
for img in img_list3:
	input_img=cv2.imread(testblur_directory +  img )
	input_img=numpy.swapaxes(input_img,0,2)

	img_data_list3.append(input_img)

img_data3 = numpy.array(img_data_list3)
img_data3 = img_data3.astype('float32')
img_data3 /= 255

print(img_data3.shape)
num_of_samples3 = img_data3.shape[0]
labels3 = numpy.ones((num_of_samples3,),dtype='int64')
print("length of labels3 is "+str(len(labels3)))
print("labels1 are all "+str(labels3[10]))

##########################################################
#laoding none blurry images

img_data_list4=[]
data_dir_list4 = os.listdir(testnoblur_directory)

img_list4=os.listdir(testnoblur_directory)
for img in img_list4:
	input_img=cv2.imread(testnoblur_directory +  img )
	input_img = numpy.swapaxes(input_img, 0, 2)
	img_data_list4.append(input_img)

img_data4 = numpy.array(img_data_list4)
img_data4 = img_data4.astype('float32')
img_data4 /= 255
print(img_data4.shape)

num_of_samples4 = img_data4.shape[0]
labels4 = numpy.ones((num_of_samples4,),dtype='int64')
labels4[:]=0
print("length of labels4 is "+str(len(labels4)))
print("labels1 are all "+str(labels4[10]))
#######################################################
# Combine the two numpy arrays and shuffle
labels=numpy.concatenate((labels3,labels4),axis=0)
test_image = numpy.concatenate((img_data1,img_data2),axis=0)
